...
class Parser
{
    ...
    public function parse(): void
    {
        ...
        while (!feof($this->stream) && !$eof) {
            ...
            // set the underlying streams chunk size, so it delivers according t
o the request from stream_get_line
            stream_set_chunk_size($this->stream, $this->bufferSize);
            $line = stream_get_line($this->stream, $this->bufferSize, $this->lin
eEnding);
...
e
            ...
            for ($i = 0; $i < $byteLen; $i++) {
                if ($this->listener instanceof PositionAwareInterface) {
                    $this->listener->setFilePosition($this->lineNumber, $this->c
harNumber);
                }
    ...
    private function consumeChar(string $char): void
    {
        ...
        // thanks: http://stackoverflow.com/questions/16042274/definition-of-whi
tespace-in-json
        if ((' ' === $char || "\t" === $char || "\n" === $char || "\r" === $char
) &&
            ...
            // we wrap this so that we don't make a ton of unnecessary function
calls
        ...
        switch ($this->state) {
            case self::STATE_IN_STRING:
                if ('"' === $char) {
                    ...
                    $this->throwParseError('Unescaped control character encounte
red: '.$char);
                } else {
                    $this->buffer .= $char;
                }
            ...
            case self::STATE_IN_OBJECT:
                if ('}' === $char) {
                    ...
                    $this->throwParseError('Start of string expected for object
key. Instead got: '.$char);
                }
                break;
            ...
            case self::STATE_AFTER_VALUE:
                ...
                if (self::STACK_OBJECT === $within) {
                    if ('}' === $char) {
                        ...
                        $this->throwParseError("Expected ',' or '}' while parsin
g object. Got: ".$char);
                    }
            ...
            case self::STATE_IN_NUMBER:
                if (ctype_digit($char)) {
                    ...
                    if (false !== strpos($this->buffer, '.')) {
                        $this->throwParseError('Cannot have multiple decimal poi
nts in a number.');
                    } elseif (false !== stripos($this->buffer, 'e')) {
                        $this->throwParseError('Cannot have a decimal point in a
n exponent.');
                    }
                    $this->buffer .= $char;
                } elseif ('e' === $char || 'E' === $char) {
                    if (false !== stripos($this->buffer, 'e')) {
                        $this->throwParseError('Cannot have multiple exponents i
n a number.');
                    }
                    $this->buffer .= $char;
                } elseif ('+' === $char || '-' === $char) {
                    ...
                    if (!('e' === $last || 'E' === $last)) {
                        $this->throwParseError("Can only have '+' or '-' after t
he 'e' or 'E' in a number.");
                    }
                    $this->buffer .= $char;
                } else {
                    $this->endNumber();
            ...
            case self::STATE_START_DOCUMENT:
                ...
                if ('[' === $char) {
                    ...
                    $this->throwParseError('Document must start with object or a
rray.');
                }
                break;
            ...
            default:
                $this->throwParseError('Internal error. Reached an unknown state
: '.$this->state);
                break;
        }
    }
    ...
    private function processEscapeCharacter(string $c): void
    {
        if ('"' === $c) {
            ...
            $this->throwParseError('Expected escaped character after backslash.
Got: '.$c);
        }
    ...
    private function processUnicodeCharacter(string $char): void
    {
        if (!ParserHelper::isHexCharacter($char)) {
            $this->throwParseError(
                ...
                .'Unicode parsed: '.implode('', $this->unicodeBuffer).' and got:
 '.$char
            );
        }
        ...
        if (4 === \count($this->unicodeBuffer)) {
            ...
            if ($codepoint >= 0xD800 && $codepoint < 0xDC00) {
                ...
                if (-1 === $this->unicodeHighSurrogate) {
                    $this->throwParseError('Missing high surrogate for Unicode l
ow surrogate.');
                }
                $combinedCodepoint = (($this->unicodeHighSurrogate - 0xD800) * 0
x400) + ($codepoint - 0xDC00) + 0x10000;

                $this->endUnicodeCharacter($combinedCodepoint);
            } else {
                if (-1 !== $this->unicodeHighSurrogate) {
                    $this->throwParseError('Invalid low surrogate following Unic
ode high surrogate.');
                } else {
    ...
    private function endUnicodeSurrogateInterstitial(): void
    {
        ...
        if ('\\u' !== $unicodeEscape) {
            $this->throwParseError("Expected '\\u' following a Unicode high surr
ogate. Got: ".$unicodeEscape);
        }
    ...
    {
        throw new ParsingException($this->lineNumber, $this->charNumber, $messag
e);
    }
}
